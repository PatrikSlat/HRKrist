...
app.get('/reminders', (req, res) => {
    connection.query('SELECT * FROM podsjetnik', (err, results) => {
        if (err) {
...
app.post('/reminders', (req, res) => {
    ...
    const query = 'INSERT INTO podsjetnik (title, description) VALUES (?, ?)';
...
//INTENTIONS
app.get('/nakane', (req, res) => {
    connection.query('SELECT * FROM nakana', (err, results) => {
        if (err) {
            res.status(500).send(err);
            return;
        }
...
app.post('/nakane', (req, res) => {
    const { opis } = req.body;
    const query = 'INSERT INTO nakana (opis) VALUES (?)';
    connection.query(query, [opis], (err, result) => {
        if (err) {
            res.status(500).send(err);
            return;
        }
        res.send({ id: result.insertId, opis });
    });
});
